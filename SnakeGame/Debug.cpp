...
std::array<Line, MAX_CELL_DIM * MAX_CELL_DIM + 2> getGridLines(int cell_count_x, int cell_count_y, float cell_size) {
    ...
    for (int i = 0; i <= cell_count_x; i++) {
        lines[i] = Line(glm::vec3(i * cell_size - offset_x, -offset_y, 0.0f), glm::vec3(i * cell_size - offset_x, cell_size * cell_count_y - offset_y, 0.0f), glm::vec3(1.0f, 1.0f, 1.0f));
    }
    for (int j = 0; j <= cell_count_y; j++) {
        lines[cell_count_x + 1 + j] = Line(glm::vec3(-offset_x, j * cell_size - offset_y, 0.0f), glm::vec3(cell_size * cell_count_x - offset_x, j * cell_size - offset_y, 0.0f), glm::vec3(1.0f, 1.0f, 1.0f));
    }
